...
export default class DatabaseHelper {
  ...
  constructor(filePath: string) {
    ...
    if (!DatabaseHelper.getBalStatement)
      DatabaseHelper.getBalStatement = this._db.prepare(
        'SELECT cash, bank FROM balance WHERE userID=? AND serverID=?'
      );

    if (!DatabaseHelper.getAllUserBalsStatement)
      DatabaseHelper.getAllUserBalsStatement = this._db.prepare(
        'SELECT userID, serverID, cash, bank FROM balance WHERE userID=?'
      );

    if (!DatabaseHelper.getAllServerBalsStatement)
      DatabaseHelper.getAllServerBalsStatement = this._db.prepare(
        'SELECT userID, serverID, cash, bank FROM balance WHERE serverID=?'
      );

    if (!DatabaseHelper.setBalStatement)
      DatabaseHelper.setBalStatement = this._db.prepare(
        'INSERT INTO balance (userID, serverID, cash, bank) VALUES (@userID, @serverID, @cash, @bank) ON CONFLICT(userID, serverID) DO UPDATE SET cash=@cash, bank=@bank'
      );
  }
